...
Begin VB.Form Frm_Streams 
   BackColor       =   &H00CFE1E2&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "App Title : Streams"
   ClientHeight    =   2430
   ClientLeft      =   45
   ClientTop       =   735
   ClientWidth     =   5175
   Icon            =   "Frm_Streams.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   2430
   ...
   Begin VB.Frame Fra_Classes 
      ...
      Begin VB.TextBox txtCapacity 
         ...
         Width           =   420
      End
      Begin VB.TextBox txtStreamName 
         BackColor       =   &H00C0FFFF&
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
...
On Local Error GoTo Handle_ClearEntries_Error
    ...
    txtStreamName.Tag = VBA.vbNullString
    txtStreamName.Text = VBA.vbNullString
    txtCapacity.Text = &H0
...
On Local Error GoTo Handle_DisplayRecord_Error
    ...
    With vRs 'Execute a series of statements on vRs recordset
        ...
        .Open "SELECT * FROM [Qry_Streams] WHERE [Stream ID] = " & vRecordID, vAdoCNN, adOpenKeyset, adLockReadOnly
        ...
        If Not (.BOF And .EOF) Then
            ...
            txtStreamName.Tag = ![Stream ID] 'Assign the Record's primary key value
            
            If Not VBA.IsNull(![Class ID]) Then txtClassName.Tag = ![Class ID]
            If Not VBA.IsNull(![Class Name]) Then txtClassName.Text = ![Class Name]
            If Not VBA.IsNull(![Stream Name]) Then txtStreamName.Text = ![Stream Name]
...
On Local Error GoTo Handle_chkDiscontinued_Click_Error
    ...
    If chkDiscontinued.Value = vbChecked Then If vMsgBox("Ticking this option will disable the Stream and will not be available in other Modules. Proceed?", vbQuestion + vbYesNo + vbDefaultButton2, App.Title & " : Confirmation", Me) = vbNo Then myRecDisplayON = True: chkDiscontinued.Value = vbUnchecked: myRecDisplayON = False
...
On Local Error GoTo Handle_CmdMoveRec_Click_Error
    ...
    myRecIndex = NavigateToRec(Me, "SELECT * FROM [Qry_Streams] ORDER BY [Class Level] ASC, [Stream Name] ASC", "Stream ID", Index, myRecIndex)
...
On Local Error GoTo Handle_CmdAddClass_Click_Error
    ...
    If VBA.LenB(VBA.Trim$(txtClassName.Tag)) <> &H0 Then vEditRecordID = txtClassName.Tag & "|||"
    Call LoadUnloadedFormViaStringName(Me, Frm_FamilyRelationships.Name, "Class:Classes:Class:Classes:Tbl_Classes:Tbl_Streams:Streams")
    If vDatabaseAltered And Fra_Classes.Enabled Then vDatabaseAltered = False: Call cmdClearClass_Click
    
Exit_CmdAddClass_Click:
...
Handle_CmdAddClass_Click_Error:
    
    'Indicate that a process or operation is complete.
    Screen.MousePointer = vbDefault
    
    'Warn User
    vMsgBox Err.Description, vbExclamation, App.Title & " : Error Loading Class Form - " & Err.Number, Me
    
    'Resume execution at the specified Label
    Resume Exit_CmdAddClass_Click
...
On Local Error GoTo Handle_cmdSelectClass_Click_Error
    ...
    vBuffer(&H0) = PickDetails(Me, "SELECT [Class ID], [Class Name] FROM [Tbl_Classes] WHERE [Discontinued] = FALSE ORDER BY [Hierarchical Level] ASC", txtClassName.Tag, "1;2", "1", , , "Classes", , , , 4500)
    ...
    txtClassName.Tag = vArrayList(&H0)  'Assign Class ID
...
On Local Error GoTo Handle_Form_Activate_Error
    ...
    With vRs 'Execute a series of statements on vRs recordset
        ...
        If vEditRecordID <> VBA.vbNullString Then
            ...
            .Filter = "[Stream ID] = " & vArrayList(&H0)
            ...
            If nHasRecords Then DisplayRecord VBA.CLng(VBA.Val(![Stream ID]))
...
On Local Error GoTo Handle_MnuDelete_Click_Error
    ...
    If txtStreamName.Tag = VBA.vbNullString Then
    ...
    'Confirm deletion
    If vMsgBox("Are you sure you want to DELETE the displayed Stream?", vbQuestion + vbYesNo + vbDefaultButton2, App.Title & " : Confirmation", Me) = vbNo Then
    ...
    iDependencyArray = VBA.Split("[Tbl_StudentClasses]", "|")
    ...
    For vIndex(&H0) = &H0 To UBound(iDependencyArray) Step &H1
        ...
        iDependency = CheckForRecordDependants(Me, iDependencyArrayTmp(&H0) & " WHERE [Stream ID] = " & VBA.Val(txtStreamName.Tag))
    ...
    If iDependency Then
        ...
        'Warn User
        vMsgBox "The displayed Stream has other Records depending on it {" & iDependencyArrayTmp(&H1) & "}. Delete operation aborted", vbExclamation, App.Title & " : Operation Aborted", Me
    ...
    vRs.Open "DELETE * FROM " & myTable & " WHERE [Stream ID] = " & VBA.Val(txtStreamName.Tag), vAdoCNN, adOpenKeyset, adLockPessimistic
    ...
    'Inform User
    vMsgBox "The Stream has successfully been deleted.", vbInformation, App.Title & " : Delete", Me
...
Private Sub MnuEdit_Click()
    ...
    If txtStreamName.Tag = VBA.vbNullString Then
        ...
        'Warn User to select an existing Record
        vMsgBox "Please display a Record.", vbExclamation, App.Title & " : No Record Displayed", Me
        If Fra_Classes.Enabled Then txtStreamName.SetFocus 'Move the focus to the specified control
    ...
    txtStreamName.SetFocus 'Move focus to Name textbox
...
On Local Error GoTo Handle_MnuSave_Click_Error
    ...
    If txtStreamName.Text = VBA.vbNullString Then
        ...
        txtStreamName.SetFocus 'Move the focus to the specified control
    ...
    txtStreamName.Text = CapAllWords(VBA.Trim$(VBA.Replace(txtStreamName.Text, "  ", " ")))
    ...
    With vRs 'Execute a series of statements on vRs recordset
        ...
        If VBA.Val(txtCapacity.Text) <> &H0 Then
            ...
            .Open "SELECT * FROM [Tbl_StudentClasses] WHERE [Stream ID] = " & VBA.Val(txtStreamName.Tag), vAdoCNN, adOpenKeyset, adLockPessimistic
            ...
            If .RecordCount > VBA.Val(txtCapacity.Text) Then
                ...
                vMsgBox "The entered Stream Capacity is lower than the total number of Students already assigned to it {" & .RecordCount & "}. Please set a capacity of equal or higher figure, or set to zero {0} for unlimited capacity.", vbExclamation, App.Title & " : Invalid Capacity Limit", Me
        ...
        'Check if the entered Stream name already exists in the database
        .Filter = "[Class ID] = " & VBA.Val(txtClassName.Tag) & " AND [Stream ID] <> " & VBA.Val(txtStreamName.Tag) & " AND [Stream Name] = '" & VBA.Replace(txtStreamName.Text, "'", "''") & "'"
        ...
        If Not (.BOF And .EOF) Then
            ...
            vMsgBox "The entered Stream name had already been saved for the selected Class. Please enter a different name.", vbExclamation, App.Title & " : Duplicate Entry", Me
            txtStreamName.SetFocus 'Move the focus to the specified control
            GoTo Exit_MnuSave_Click 'Quit this saving procedure
        ...
        If Not IsNewRecord Then .Filter = "[Stream ID] = " & txtStreamName.Tag: .Update Else .AddNew
        
        'Assign entries to their respective database fields
        
        ![Class ID] = VBA.Val(txtClassName.Tag)
...
On Local Error GoTo Handle_MnuSearch_Click_Error
    ...
    'Enable User to select a Record
    vBuffer(&H0) = PickDetails(Me, "SELECT [Stream ID], ([Class Name] & ' ' & [Stream Name]) AS [Class] FROM [Qry_Streams] ORDER BY [Class Level] ASC, [Stream Name] ASC", txtStreamName.Tag, , "1", , , "Streams")
...
Private Sub ShpBttnBriefNotes_Click()
    ...
    Call OpenNotes(ShpBttnBriefNotes, Not Fra_Classes.Enabled, "Stream")
End Sub

Private Sub txtCapacity_Change()
    'Prevent empty content
    txtCapacity.Text = VBA.IIf(txtCapacity.Text = VBA.vbNullString, udCapacity.Min, txtCapacity.Text)
    txtCapacity.SelStart = VBA.Len(txtCapacity.Text)
End Sub
